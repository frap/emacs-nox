...
**** From DOOM Emacs
gsetq is from general.el
#+BEGIN_SRC emacs-lisp
(defmacro gen-setq (&rest settings)
...
(defun gas-try-run-hook (hook)
  "Run HOOK (a hook function), but marks thrown errors to make it a little
easier to tell where they came from.

Meant to be used with `run-hook-wrapped'."
  (when gas-debug-mode
    (message "Running gas hook: %s" hook))
  (condition-case e
      (funcall hook)
    ((debug error)
     (signal 'gas-hook-error (list hook e))))
...
compilation. This will no-op on features that have been disabled by the user."
  ...
  (unless (and (symbolp targets)
               (memq targets (bound-and-true-p gas-disabled-packages)))
...
*** Hooks
#+BEGIN_SRC emacs-lisp
(defvar gas-post-init-hook nil
  "A list of hooks to run when Emacs is fully initialised. Fires at the end of
`emacs-startup-hook', as late as possible. Guaranteed to run after everything
else (except for `window-setup-hook').")

(defun gas|post-init ()
  "Run `gas-post-init-hook'."
  (run-hook-wrapped 'gas-post-init-hook #'gas-try-run-hook))
(add-hook! 'emacs-startup-hook #'gas|post-init)
#+END_SRC

**** Window and buffer switch
#+BEGIN_SRC emacs-lisp
(defvar gas-before-switch-window-hook nil
  "Hook run before `switch-window' or `switch-frame' are called. See
`gas-after-switch-window-hook'.")

(defvar gas-after-switch-window-hook nil
  "Hook run after `switch-window' or `switch-frame' are called. See
`gas-before-switch-window-hook'.")

(defvar gas-before-switch-buffer-hook nil
...
(defvar gas-after-switch-buffer-hook nil
...
(defun gas*switch-window-hooks (orig-fn window &optional norecord)
  (if (or gas-inhibit-switch-window-hooks
    ...
    (let ((gas-inhibit-switch-window-hooks t))
      (run-hooks 'gas-before-switch-window-hook)
      (prog1 (funcall orig-fn window norecord)
        (with-selected-window window
          (run-hooks 'gas-after-switch-window-hook))))))
(defun gas*switch-buffer-hooks (orig-fn buffer-or-name &rest args)
  (if (or gas-inhibit-switch-buffer-hooks
    ...
    (let ((gas-inhibit-switch-buffer-hooks t))
      (run-hooks 'gas-before-switch-buffer-hook)
      (prog1 (apply orig-fn buffer-or-name args)
        (with-current-buffer buffer-or-name
          (run-hooks 'gas-after-switch-buffer-hook))))))
...
(add-hook 'gas-post-init-hook #'gas|setup-switch-hooks)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(require 'package)
(setq-default package-user-dir      gas-packages-dir
...
(use-package quelpa :ensure
  :init
  (setq-default quelpa-dir (concat gas-packages-dir "quelpa-cache/")
...
**** Files
#+BEGIN_SRC emacs-lisp
(gsetq abbrev-file-name               (concat gas-local-dir "abbrev.el")
...
(cond (IS-MAC
       ...
       (when (or (daemonp) (not (display-graphic-p)))
         (add-hook 'gas-post-init-hook #'osx-clipboard-mode))
...
#+BEGIN_SRC emacs-lisp
(use-package hl-line
  :ghook ('gas-post-init-hook #'global-hl-line-mode)
  :config
...
*** Font
#+BEGIN_SRC emacs-lisp
(defvar gas-fonts '(("PragmataPro" . 12) ("Hack" . 11) ("DejaVu Sans Mono" . 11)
...
(defun gas/change-font ()
  ...
  (let* (available-fonts font-name font-size font-setting)
    ...
    (when gas-debug-mode (message "Available fonts: %s" available-fonts))
    ...
    (cond ((string= font-name "PragmataPro")
           (gas-font-set-faces `(org-table ((t (:family ,(format "PragmataPro Mono-%d" font-size))))))))))
#+END_SRC

*** Theme
**** Theme customizer
#+BEGIN_SRC emacs-lisp
(defvar gas-customize-theme-hook nil
...
(defun gas*customize-theme (theme)
  ...
  (run-hook-with-args-until-success 'gas-customize-theme-hook (or theme
                                                                  (car custom-enabled-themes))))
(advice-add 'enable-theme :after #'gas*customize-theme)

(defmacro gas-add-customize-theme-hook (target-theme &rest body)
  "TARGET-THEME may be a list, a symbol or a regexp."
  (declare (indent defun))
  `(add-hook 'gas-customize-theme-hook
             (lambda (theme)
...
(use-package doom-themes :ensure
  ...
  (gas-add-customize-theme-hook "^doom-"
...
(use-package color-theme-sanityinc-tomorrow :ensure
  :config
  (gas-add-customize-theme-hook "^sanityinc-"
    (custom-theme-set-faces
     theme
     `(org-special-keyword ((t (:inherit shadow)))))))
...
(use-package solarized :ensure solarized-theme
  ...
  (gas-add-customize-theme-hook 'solarized-dark
  ...
  (gas-add-customize-theme-hook 'solarized-light
...
(add-hook! '(gas-post-init-hook minibuffer-setup-hook window-configuration-change-hook)
...
(use-package company-dabbrev
  ...
  (defun gas-company-dabbrev-buffer-check (buffer)
    (with-current-buffer buffer (derived-mode-p 'pdf-view-mode
                                                'doc-view-mode)))
  (gsetq company-dabbrev-ignore-buffers #'gas-company-dabbrev-buffer-check))
#+END_SRC
...
(use-package projectile :ensure
 ...
 ;; :general ("C-c p" '(:keymap projectile-command-map :wk "Projectile"))
  ...
  :init
  (gsetq projectile-cache-file (concat gas-cache-dir "projectile.cache")
...
(use-package dumb-jump :ensure
  ...
  (setq dumb-jump-selector 'ivy
        dumb-jump-default-project gas-emacs-dir
...
;;  :general (gas-leader "i" 'imenu-anywhere)
  :custom
...
(use-package cc-mode
  ...
  (defun gas-c-hook ()
    (c-set-style "GasStyle")
    (c-toggle-auto-hungry-state -1)
    (if (and buffer-file-name
             (not (file-exists-p buffer-file-name))
             (string-match "\\.h\\(pp\\)?\\'" buffer-file-name))
        (gas-header-format)))
  (add-hook! c-mode-common #'gas-c-hook)
...
;;  (add-hook! :local clojure-mode 'subword-mode)
;;  (add-hook! :local clojure-mode 'global-prettify-symbols-mode)
  (defun figwheel-repl ()
   (interactive)
...
(use-package org
 ...
 :bind (("C-c l" . org-store-link)
  ...
  :config
  (gsetq org-modules '(org-habit org-id org-protocol org-timer org-bullets))
...
(use-package org-attach
  :config
  (gsetq org-attach-directory "Ressources/attacher/"
...
(use-package org-noter :ensure
  ...
  :config
  (gsetq org-noter-default-heading-title "Notes de page $p$"
...
(use-package deft :ensure
 ;; :general (gas-leader "d" '(deft :wk "Deft"))
  :config
  (gsetq deft-directory "~/Dropbox/GTD/Deft/"
...
(use-package treemacs :ensure
  :init
  (gsetq treemacs-persist-file (concat gas-cache-dir "treemacs-persist")))
#+END_SRC
